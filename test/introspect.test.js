...
const testLocalFileOutput = './temporary-test-outputs/localwithfilter.json'
const defaultRemoteTestURL = 'https://api.sciencebase.gov/nabatmonitoring-survey/graphql'//'http://localhost:5000/graphql'
const testRemoteFileOutput = './temporary-test-outputs/remoteSchema.json'
const testFilteredRemoteFileOutput ='./temporary-test-outputs/filteredRemoteSchema.json'
...
describe('Load Schema From Local URL', () => {
  test('load schema with introspection', () => {
    return fetch(defaultLocalTestURL, {
      ...
      .catch(error => console.error('Error: ', error))
      .then((result) => {
        fs.writeFileSync(testLocalFileOutput, JSON.stringify(result.data, null, 2))
      })
      .catch(error => console.error('Error2: ', error))
  })
})
...
test('filter schema field names', () => {
  let rawdata = fs.readFileSync(testRemoteFileOutput)
...
//  console.log(`mutationFilterStrings: ${JSON.stringify(mutationFilterStrings, null, 2)}`)
  ...
  fs.writeFileSync(testFilteredRemoteFileOutput, JSON.stringify(filtered, null, 2))
